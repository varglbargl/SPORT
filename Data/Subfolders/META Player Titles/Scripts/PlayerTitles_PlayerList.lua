...
--  nil UpdatePlayerScore()
--  Updates the little player score on the player entry on resourceChangedEvent
local function UpdatePlayerScore(player, resourceName, amount)
  if resourceName == "Score" then
    local entry = Entries:FindChildByName(player.name)
    if(not entry) then return end

    local playerScoreText = entry:GetCustomProperty("PlayerScore"):WaitForObject()
    playerScoreText.text = tostring(amount)
  end
end
...
local function CreatePlayerEntry(player)
	...
	local playerNameText, teamColorImage, playerIconImage, socialIconImage, playerScoreText =
    ...
    entry:GetCustomProperty("PlayerScore"):WaitForObject()
  ...
  player.resourceChangedEvent:Connect(UpdatePlayerScore)
