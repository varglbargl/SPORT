...
Utils.realGoalList = {
  ...
  },
  {
    ...
    test = function(player, goalIndex, goalID, reward) xEventsForPlayer(player, goalIndex, goalID, reward, 1, "TargetShoot", "Tuba Ball") end,
    reward = 10
  },
  {
    id = 8,
...
Utils.fakeGoalList = {
  ...
  },
  {
    ...
    test = function(player, goalIndex, goalID, reward, roundLength) xEventsForPlayer(player, goalIndex, goalID, reward, 1, "ItsOutaHere", nil) end,
    reward = 5
  },
  {
    id = 104,
...
function xEventsForPlayer(player, goalIndex, goalID, reward, x, eventName, withBallType, withPointValue)
  ...
  function checkGoal(scoringPlayers, isAPlayer, ballName, pointValue)
    ...
    if withBallType == "Player" and not isAPlayer then
      return
    elseif withBallType and withBallType ~= ballName then
      return
    end
...
function Utils.getGoalById(id)
  local result = nil

  if id > 100 then
    for _, goal in ipairs(Utils.fakeGoalList) do
      if goal.id == id then
        result = goal
        break
      end
    ...
    for _, goal in ipairs(Utils.realGoalList) do
      if goal.id == id then
        result = goal
        break
      end
